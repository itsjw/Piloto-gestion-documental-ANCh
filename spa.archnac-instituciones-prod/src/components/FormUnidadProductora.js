...
class FormUnidadProductora extends Component {
    ...
    render(){
        const { node, mode, intl, valid, handleSubmit } = this.props
        return (
            <form className="card" onSubmit={ handleSubmit(this.onSubmit) }>
                <div className="card-header ch-alt">
                    <h2>
                        Formulario: Unidad Productora
                ...
                <div className="card-body card-padding">
                    ...
                    <div className="row">
                        <div className="col-sm-12">
                            <Field 
                                name="achn_area_relaciones" 
                                component={ TextAreaField } 
                                fieldProps={{ rows: 10 }} 
                                placeholder="Breve resumen sobre el área de relaciones"
                            />
                        </div>
    ...
    componentDidMount(){
        ...
        if(mode === 'edit'){
            initialize({ 
                ...
                achn_aggr_fch_existencia: moment(node.properties['achn:aggr_fch_existencia']).format('YYYY/MM/DD HH:mm')
            })
        } else {
            initialize({
                achn_aggr_nivel_clasificacion: 'Unidad Productora',
                achn_archive_codigo: condFUndef(() => `${getCod(selectedNode)}<codigo>`, selectedNode),
                achn_aggr_fch_existencia: moment().format('YYYY/MM/DD HH:mm'),
                achn_aggr_tipo: 'SECCION',
                cm_created: moment().format(),
                achn_archive_descripcion: "not used"
            })
        }
    }
...
)(reduxForm({ form: 'producerInst' })(injectIntl(FormUnidadProductora)))