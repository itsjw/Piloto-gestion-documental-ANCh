...
export default connect(state => ({ user: state.app.user, pRouteClf: state.archnac.pRouteClf }), { routerGo })(
    ({ pRouteClf, user, routerGo, ...props }) => condFNull(
        <li>
            <a onClick={ () => routerGo(PAGE_CLASSIFICATION) }>
                <div className="tm-icon" style={{ lineHeight: '12px' }}>
                    <i className="c-white tm-icon zmdi zmdi-settings"></i>
                    <small className="c-white" style={{ display: 'block' }}>
                        <FormattedMessage id="page_classification" />
                    </small>
                </div>
            </a>
        </li>, 
        hasGrpClfManager(user) && !pRouteClf 
    )
)